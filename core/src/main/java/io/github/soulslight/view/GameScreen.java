...
public final class GameScreen implements GameState {
    ...
    public void render(float delta) {
        ...
        for (AbstractEnemy enemy : model.getActiveEnemies()) {
            ...
            if (enemy instanceof SpikedBall) {
                ...
                if (sb.isCharging()) {
                    float offset =
                        enemyAnimOffset.computeIfAbsent(enemy, e -> MathUtils.random(0f, 10f));
    ...
    private void drawEntity(TextureRegion region, Vector2 pos, float width, float height, boolean flipX) {
