...
public abstract class AbstractEnemy extends Entity implements Cloneable {
    ...
    protected Vector2 lastKnownPlayerPos = new Vector2();//serve oer ricordare l'ultima posizione del player per cercarlo qualora lo perdesse
    ...
    public void createBody(World world, float x, float y) {
        ...
        shape.setRadius(this instanceof Oblivion ? 24f : 14f);// crea la hitbox, 24 per il boss, 14 per tutti gli altri
    ...
    public boolean canSeePlayer(Player player, World world) {
        ...
        world.rayCast((fixture, point, normal, fraction) -> {
            ...
            return 1;
        }, this.getPosition(), player.getPosition());
    ...
    protected void updateWanderPatrol(float deltaTime) {
        ...
        float patrolSpeed = speed * 0.3f; // i nemici sono più lenti durante la fase di pattuglia e accelerano quando vedono igiocatori
    ...
    private boolean checkObstacle(float angleOffset) {
        ...
        Vector2 rayEnd = new Vector2(1, 0) // crea un vettore unitario
            .setAngleDeg(patrolAngle + angleOffset) // indirizza il vettore nella direzione giusta grazie all'offset
    ...
    //metodo che toglie il corpo fisico dal mondo una volta morto in modo da non avere sovraccarico in memoria
