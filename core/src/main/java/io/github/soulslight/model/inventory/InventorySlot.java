...
/**
 * An Inventory Slot can only contain a single Item Type, though if it is stackable, many instances
 * can be stored within the same Slot.
 */
public class InventorySlot {
   ...
   * @return 'true' only if the Type and amount allow the item to be added to the current stack of
   *     this slot.
   */
  public boolean canAddStack(IStackable stackable) {
    if (!(peek() instanceof IStackable)) return false;

    return (contains(stackable) && getAmount() < stackable.getMaximumStackAmount());
  }

  /**
   * To check whether or not this InventorySlot contains a certain Type of item. Uses the class
   * names, and not references.
   *
   * @param checked An instance of the item type to be verified against.
   * @return 'true' only if the Inventory already contains at least one item of the same type as the
   *     input.
   */
  ...
  /**
   * Adds the input item, without verification of whether or not it is the same type as the other
   * items.<br>
   ...
   *
   * @return 'null' if the Slot is empty. Otherwise, returns the instance of the last Item contained
   *     in this Slot.
   */
   ...
   *
   * @return 'null' if the Slot is empty. Otherwise, returns the instance of the first Item
