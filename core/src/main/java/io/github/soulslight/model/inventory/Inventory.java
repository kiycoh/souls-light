package io.github.soulslight.model.inventory;
...
public class Inventory {
  ...
  // this.capacity = capacity;
  // this.slots = new Array<>(itemsToCopy); // todo: ensure it is a proper "clone"
  // without references to the original?
  // }
  //
  // /**
  // * An Inventory with the listed items added to it.
  ...
  // * @throws Exception thrown if there aren't enough slots to add all the items
  // listed.
  // */
  // public Inventory(int capacity, InventorySlot... slots) throws Exception {
  // if(slots.length > capacity)
  // throw new Exception("Not enough slots for all these items to be
  // initialized.");
  // this.capacity = capacity;
  ...
  // /**
  // * An Inventory with the listed items added to it.
  // * The capacity of the inventory is of the amount of Item Types it is
  // initialized with.
  // *
  // * @param slots items to add to the Inventory.
  // */
  ...
  /**
   * Removes and returns the item of the specified type from the slot which contains the least
   ...
   * Looks through the Inventory to find the Slot which contains the least amount of that item.<br>
   * In other words, if two stackable slots of the same item exist, the smallest one is returned.
   *
   * @param item The researched item.
   * @return The index that designates which InventorySlots contains the specified item. Returns
   *     "-1" if the Inventory doesn't contain such item.
   */
  ...
  public int getFreeIndex(IPickable item) {
    ...
    if (item instanceof IStackable) { // if it's Stackable, we must try to Stack first
      for (int i = 0; i < capacity; i++) {
        if (getSlot(i)
            .canAddStack((IStackable) item)) // must be proper type, and have enough space to stack
        return i;
      }
    }
