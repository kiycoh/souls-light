...
class GameManagerTest {
    ...
    void testStartCampaignResetsState() {
        ...
        assertEquals(1, gm.getCurrentLevelIndex(), "StartCampaign deve resettare l'indice del livello a 1");
        assertEquals(GameMode.STORY, gm.getGameMode(), "Il GameMode deve essere impostato correttamente");
    }
    ...
    void testClassSelection() {
        ...
        // Default
        assertEquals(Player.PlayerClass.WARRIOR, gm.getSelectedPlayerClass(), "Il default dovrebbe essere WARRIOR");

        // Cambio selezione
        gm.setSelectedPlayerClass(Player.PlayerClass.MAGE);
        assertEquals(Player.PlayerClass.MAGE, gm.getSelectedPlayerClass(), "Deve aggiornare la classe selezionata");
    }
    ...
    void testCustomModeLoop() {
        ...
        // In modalità custom , advanceToNextLevel dovrebbe sempre tornare true (loop infinito o procedurale)
        boolean result = gm.advanceToNextLevel();
